...
## `POST /wallet/deduct` — ⚠ لم يعد يخصم (تحديث 2026-09-08)

**هذه النقطة صارت فحص رصيد لا حركة مال.** الاستدعاء لا يغيّر الرصيد إطلاقاً.

```json
// Request
{ "amount": 22.5, "label": "دفع طلب #ORD-M3K2" }
// 200 — الرصيد يكفي، ولم يُخصم شيء
{ "balance": 82.75, "sufficient": true }
// 409 — رصيد غير كافٍ
...
`POST /orders` يخصم المحفظة **داخل نفس معاملة قاعدة البيانات** التي تكتب الطلب.
فالفرونت الذي يستدعي `deduct` أولاً ثم ينشئ الطلب كان:

- **يخصم مرتين** عند نجاح الاثنين
- **يخصم بلا طلب** عند فشل الثاني بعد نجاح الأول

والثانية وقعت فعلاً على عميل (0595796456) في 2026-09-07: خُصم رصيده ولم يُنشأ طلب،
واحتاج تعويضاً يدوياً من اللوحة.

جعلُها فحصاً بدل حذفها يُبقي الفرونت الحالي يعمل **بلا أي تعديل**: يستدعيها، يرى
نجاحاً، ينشئ الطلب، فيُخصم مرة واحدة. ولا يبقى أي طريق لتحريك المال بلا طلب
مرتبط به.

### ما ننصح به للفرونت

احذفوا الاستدعاء كلياً. `POST /orders` بـ`paymentMethod: "wallet"` يتحقق من
الرصيد تحت قفل صف ويخصم وينشئ الطلب في معاملة واحدة — إن فشل أي جزء تراجع الكل.
ولا حاجة لمنطق الاسترداد عند الفشل الموجود في `PaymentClientPage.tsx`، فلا شيء
يُخصم كي يُسترد.
...
- [x] لا يمكن خصم رصيد بلا طلب مرتبط به — الخصم يحدث داخل معاملة إنشاء الطلب وحدها
