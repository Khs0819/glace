...
### الملاحظتان: `orderNote` و `captainNote` (محدّث 2026-09-15)

حقلان اختياريان (نص حر، حتى 1000 حرف)، **منفصلان ولا يُدمجان**:

| الحقل | مصدره بالفرونت | لمن | أين يظهر |
|---|---|---|---|
| `orderNote` | ملاحظة السلة | من يحضّر الطلب | «ملاحظة الطلب» في لوحة الطلبات، الفاتورة، شاشة الكاشير |
| `captainNote` | ملاحظة العنوان | من يوصّل الطلب | «ملاحظة للكابتن» — للتوصيل فقط في الفاتورة والشاشة |

يُرجَعان بالاسمين نفسيهما في `GET /orders` و `GET /orders/{id}`. الحقل القديم `notes`
ما زال مقبولاً ويُرجَع، وقيمته هي نفسها `orderNote`.

> قبل هذا التحديث كان الحقلان **يُحذفان بصمت**: الطلب يمر عبر `validated()` التي تُسقط
> أي حقل بلا قاعدة تحقق، ولم تكن لأيٍّ منهما قاعدة.
...
|---|---|
| `dine-in` | قيد المراجعة ← جاري التحضير ← جاهز للاستلام ← تم التسليم |
| `pickup` | قيد المراجعة ← جاري التحضير ← جاهز للاستلام ← تم التسليم |
| `delivery` | قيد المراجعة ← جاري التحضير ← جاهز للاستلام ← في الطريق ← تم الاستلام |

> **محدّث 2026-09-15:** أُضيفت «جاري التحضير» و«جاهز للاستلام» إلى `dine-in`، و«جاهز للاستلام» إلى `delivery`
> قبل «في الطريق». على الفرونت تحديث `src/lib/orderStatusSteps.ts` ليرسم الخطوات الجديدة، وإلا ظهرت حالة لا
> يعرف شريط التتبع مكانها. «في الطريق» لا تُعطى لطلب توصيل إلا بعد اختيار سائق.
...
## `senderAccountName` — اسم صاحب الحساب المحوِّل (محدّث 2026-09-15)

نص حر حتى 190 حرفاً. **إجباري** لطرق التحويل اليدوي `jawwal-manual` و`palpay` و`bop`، سواء
رُفعت صورة الإيصال أو لا — والطلب بدونه يُرفض بـ 422 على الحقل `senderAccountName`.
منفصل تماماً عن `receiptNote` الذي يبقى اختيارياً كبديل لصورة الإيصال.

| Endpoint | السلوك |
|---|---|
| `POST /orders` | إجباري لطرق التحويل الثلاث، ولا يُخزَّن لغيرها |
| `POST /orders/{id}/receipt` | إجباري، ويحدّث القيمة على الطلب |
| `POST /wallet/topup-requests` | إجباري لطرق التحويل الثلاث (لا يُطلب لـ `jawwal` الآلي) |

يُرجَع بالاسم نفسه في `GET /orders` و`GET /orders/{id}` و`GET /wallet/topup-requests`، ويظهر
«حُوّل من حساب» في لوحة الطلبات، ولوحة طلبات الشحن، ونافذة تفاصيل الطلب في شاشة الكاشير.
